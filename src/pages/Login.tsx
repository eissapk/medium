...
const submitBtnStyle = "flex items-center gap-x-2 px-6 py-2 mt-4 mx-auto block text-sm text-green transition-all rounded-full border-green border font-medium";
...
function Login() {
	...
	const errorElement = (text: string | boolean, className = "") => (
		<div className={`p-2.5 bg-red-light border border-border-light rounded mt-4 text-center text-sm text-text-light ${className}`}>{text}</div>
	);
	return (
