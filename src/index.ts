...
if (!isDev) app.use(express.static(path.resolve(__dirname, "../client"))); // for production -- comment if you use frontend seperatly
...
if (!isDev) app.get("*", (req, res) => res.sendFile(path.resolve(__dirname, "../client/index.html"))); // for production -- comment if you use frontend seperatly

// static files
app.use("/api/assets/images", express.static(path.resolve(__dirname, "./assets/images")));
if (!isDev) app.use("/api/uploads", express.static(path.resolve(__dirname, "./uploads"))); // todo: fix this (doesn't work on production)
